function [trainedClassifier  predictedClassesTest F ACC]=naiveBayesianClassifier(features,group,sortedFeatureIndices,bestN,train,test,crossValidationMethod)
%naiveBayesianClassifier function trains naive bayesian classifier based on
...
switch crossValidationMethod
    case 1
        ...
        for i = 1 : foldNum
            trainedClassifier{i} = NaiveBayes.fit(features(train{i},sortedFeatureIndices{i}(1:bestN{i})),group(train{i}));
            predictedClassesTest{i,1} = trainedClassifier{i}.predict(features(test{i},sortedFeatureIndices{i}(1:bestN{i})));
